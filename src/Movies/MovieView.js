...
const MovieView = ({movie, onOpen}) => {
    const {openDt , moviePrdtStat, movieType, watchGradeNm, showTs, repNationCd,
         director, genre, dtNm, thumbUrl, movieNm} = movie;
    return (
        <div className="view" onClick = {onOpen}>
            <img src={thumbUrl} alt ={movieNm} style ={{cursor:'pointer'}}/>
            <div>
                <h3>{movieNm}</h3>
                <ul>
